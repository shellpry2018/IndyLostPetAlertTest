		...
		if (!is_home() && !is_front_page()) {
			...
			if ($options['breadcrumbs'] == 'enable') {
				...
				if (is_single() && get_post_type() == 'post' && !is_attachment()) {
					...
					echo __('Authors', 'mh') . $delimiter . esc_attr($user_info->display_name) . '</nav>' . "\n";
				} elseif (is_404()) {
					echo __('Page not found (404)', 'mh') . '</nav>' . "\n";
				} elseif (is_search()) {
					echo __('Search', 'mh') . $delimiter . get_search_query() . '</nav>' . "\n";
				} elseif (is_date()) {
					...
					if (is_day()) {
						...
						echo $before_link . '<a href="' . $url_year . '" title="' . __('Yearly Archives', 'mh') . '" itemprop="url">' . $before_title . $arc_year . $close_span . '</a>' . $close_span . $delimiter . $arc_month . '</nav>' . "\n";
					} elseif (is_year()) {
						echo $arc_year . '</nav>' . "\n";
					}
