	...
	function mh_featured_image() {
		...
		if (has_post_thumbnail() && $page == '1' && $options['featured_image'] == 'enable' && !get_post_meta($post->ID, 'mh-no-image', true)) {
			...
			if ($options['link_featured_image'] == 'enable') {
				...
				$att_url_begin = '<a href="' . $attachment_page . '">';
				$att_url_end = '</a>';
			} else {
				$att_url_begin = '';
				$att_url_end = '';
			...
			echo "\n" . '<div class="post-thumbnail">' . "\n";
				echo $att_url_begin . '<img src="' . $thumbnail[0] . '" alt="' . get_post_meta(get_post_thumbnail_id(), '_wp_attachment_image_alt', true) . '" title="' . get_post(get_post_thumbnail_id())->post_title . '" />' . $att_url_end . "\n";
				if ($caption_text) {
