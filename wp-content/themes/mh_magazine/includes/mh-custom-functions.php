		...
		if ($options['show_ticker']) { ?>
			<section class="news-ticker clearfix">
				<?php if ($options['ticker_title']) { ?>
					<div class="ticker-title"><?php echo esc_attr($options['ticker_title']); ?></div>
				<?php } ?>
		...
		if ($options['authorbox_layout'] != 'disable' && get_the_author_meta('description', $author_ID) && !is_attachment() || is_page_template('page-authors.php')) {
			...
			echo '<section class="author-box">' . "\n";
				echo '<div class="author-box-wrap clearfix">' . "\n";
					...
					echo '<h5 class="author-box-name">' . sprintf(__('About %s', 'mh'), esc_attr($name)) . '<span class="author-box-postcount"> (<a href="' . get_author_posts_url($author_ID) . '" title="' . sprintf(__('More articles written by %s', 'mh'), esc_attr($name)) . '">' . sprintf(__('%s Articles', 'mh'), count_user_posts($author_ID)) . '</a>)</span></h5>' . "\n";
		...
		if ($options['related_layout'] != 'disable' && $tags) {
			...
			if ($related->have_posts()) {
				...
				while ($related->have_posts()) : $related->the_post();
					...
					echo '<a href="' . $permalink . '" title="' . the_title_attribute('echo=0') . '">';
