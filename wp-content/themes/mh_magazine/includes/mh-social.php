...
if (!function_exists('mh_social_scripts')) {
	function mh_social_scripts() {
		...
		if (is_single() && $options['social_buttons'] != 'disable') {
			echo '<script src="//platform.twitter.com/widgets.js"></script>' . "\n";
			echo '<script src="https://apis.google.com/js/plusone.js"></script>' . "\n";
		}
		if (is_active_widget('', '', 'mh_facebook') || is_single() && $options['social_buttons'] != 'disable') {
			global $locale;
...
if (!function_exists('mh_share_buttons_content')) {
	function mh_share_buttons_content() {
		...
		$sb_output .= '<div class="share-button"><a href="https://twitter.com/share" class="twitter-share-button">Tweet</a></div>' . "\n";
