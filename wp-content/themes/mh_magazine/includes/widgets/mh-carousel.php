...
class mh_carousel_hp_widget extends WP_Widget {
    function mh_carousel_hp_widget() {
        $widget_ops = array('classname' => 'mh_carousel_hp', 'description' => __('Carousel widget for use on homepage template.', 'mh'));
        $this->WP_Widget('mh_carousel_hp', __('MH Carousel Widget (Homepage)', 'mh'), $widget_ops);
    }
    function widget($args, $instance) {
        ...
        echo $before_widget;
        if (!empty( $title)) { echo '<h4 class="widget-title carousel-title-' . $layout . '">' . $title . '</h4>'; } ?>
        <div id="carousel-<?php echo rand(1, 9999); ?>" class="flexslider <?php echo 'carousel-' . $layout; ?>">
			...
			while ($carousel_loop->have_posts()) : $carousel_loop->the_post(); ?>
				<li class="carousel-item">
					<a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php if (has_post_thumbnail()) { the_post_thumbnail('carousel'); } else { echo '<img src="' . get_template_directory_uri() . '/images/noimage_174x98.png' . '" alt="No Picture" />'; } ?></a>
					<div class="caption"><?php $category = get_the_category(); echo $category[0]->cat_name; ?></div>
					<?php if ($layout == 'layout2') { ?>
						<a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><h3 class="carousel-item-title"><?php the_title(); ?></h3></a>
					<?php } ?>
				</li><?php
    ...
    function form($instance) {
        $defaults = array('title' => '', 'category' => '', 'cats' => '', 'tags' => '', 'postcount' => '10', 'offset' => '0', 'order' => 'date', 'layout' => 'layout1', 'sticky' => 0);
